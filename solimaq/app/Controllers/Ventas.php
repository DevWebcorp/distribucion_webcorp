...
class Ventas extends BaseController
{
    ...
    public function get_precio()
    {
        ...
        $id_cotizacion = $request->getPost('id_cotizacions');
        $ventas_model = model('App\Models\Model_ventas\Model_ventas');
        $precios = $ventas_model->get_precios($id_cotizacion);
        ...
        $ventas_model->insert_venta($datos_venta);

        //cambiar prospecto a cliente

        $data_cotization = [
            'show_cotization' => 1,
        ];

        $ventas_model->hiden_cotization($data_cotization, $id_cotizacion);


        //insert data_ventas

        $data = [
            'tipo_persona' => $request->getPost('persona'),
            'rfc' => $request->getPost('rfc'),
            'correo' => $request->getPost('correo'),
            'telefono_fijo' => $request->getPost('telefono'),
            'telefono_movil' => $request->getPost('movil'),
            'dom_entrega' => $request->getPost('entrega'),
            'dom_facturacion' => $request->getPost('facturacion'),
            'rep_legal' => $request->getPost('representante'),
            'num_acta_constitutiva' => $request->getPost('acta'),
            'Fecha' => date("Y-m-d h:i:s"),
            'nombre' => $request->getPost('nombre'),
            'notorio_puplico' => $request->getPost('notario'),
            'id_data_clients' => $request->getPost('id_client'),

        ];

        $ventas_model->insert_dataventa($data);
    ...
    /*Tabla de detalle para generar pdf*/
    public function  tabla_detalle_view($id_venta)
    {
        ...
        $mpdf = new \Mpdf\Mpdf();
        $mpdf->WriteHTML($this->body_html($data_ventas));
        $mpdf->Output();
    }


    public function body_html($data)
    {
        return view('Ventas/Ventas_view_pdf', $data);
    }
